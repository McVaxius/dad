...
public sealed class DadModuleRegistry
{
    ...
    public DadModuleRegistry()
    {
        capabilities = new Dictionary<DadModuleId, DadModuleCapabilitySnapshot>
        {
            ...
            [DadModuleId.Mogtome] = BuildCapability(DadModuleId.Mogtome, "MOGTOME", 4, true, false, true, "MOGTOME narrow DAD-owned helper IPC enabled for readiness, start, status, stop, and attempt-limited completion.", string.Empty, canStartQueue: true, canTrackCompletion: true, canRequeue: true, canExecuteLiveQueue: true),
