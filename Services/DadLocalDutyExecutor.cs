    ...
    DadCombatRotationService combatRotationService) : IDadModuleExecutor
{
    ...
    private string BuildDutyCompleteWaitingForExitSummary()
    {
        ...
        return rotationMode switch
        {
            DadCombatRotationMode.UseFrenRider => $"Local Duty {dutyName} completed; waiting for FrenRider or user to leave. Disable commands are reserved for successful final dad.Duty.Run IPC cleanup.",
    ...
    private string BuildDelayedCompletionWaitSummary(DateTime now)
    {
        ...
        return $"Local Duty {dutyName} exited without matching DutyCompleted; waiting for delayed completion ({remaining:F0}s).";
    }
    ...
    private string BuildCompletedSummary()
    {
        ...
        return rotationMode switch
        {
            DadCombatRotationMode.UseFrenRider => $"Local Duty {dutyName} completed and stabilized; normal Dad run done without disable commands. Successful final dad.Duty.Run IPC cleanup is separate.",
